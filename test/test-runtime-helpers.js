...
describe('Runtime helpers', function () {
    ...
    it('should maintains attached node', function () {
        ...
        Helpers.onNodeAttachedToParent(c1);
        expect(Fire.engine.attachedWrappersForEditor[Fire(c1).uuid]).to.be.equals(Fire(c1));

        Helpers.onNodeDetachedFromParent(c1);
        Fire.engine.emit("post-update");
        expect(Fire.engine.attachedWrappersForEditor[Fire(c1).uuid]).to.be.undefined;
    });
    ...
    it('should debounce event in one frame', function () {
        ...
        Fire.engine.attachedWrappersForEditor[Fire(c1).uuid] = 'unchanged';
        ...
        expect(Fire.engine.attachedWrappersForEditor[Fire(c1).uuid]).to.be.equals('unchanged');
        expect(Fire.engine.attachedWrappersForEditor[Fire(c2).uuid]).to.be.undefined;
    });
});
