...
describe('Editor.setDeepPropertyByPath', function() {
    ...
    describe("Testing if path contains one '.'", function () {
        ...
        it("should invoke getset", function() {
            var scale = new Fire.v2(1, 1);
            Fire.JS.getset(target, 'scale', function () {
                return new Fire.v2(scale.x, scale.y);
            }, function (value) {
                scale = value;
            });
    ...
    describe("Asset property", function () {
        ...
        it("could be set if defined as url", function (done) {
            var loadAsset = sinon.stub(Fire.AssetLibrary, "_queryAssetInfoInEditor", function (uuid, callback) {
            ...
            Editor.setDeepPropertyByPath(target, 'textureUrl', {
                uuid: DummyUuid
            }, Fire.JS.getClassName(Fire.RawTexture));

            Fire.AssetLibrary._queryAssetInfoInEditor.restore();
        });

        it("could be cleared if defined as url", function () {
            target.textureUrl = "foo.png";

            Editor.setDeepPropertyByPath(target, 'textureUrl', { uuid: "" }, Fire.JS.getClassName(Fire.RawTexture));

            expect(target.textureUrl).to.be.equal(null);
        });

        it("could be changed if defined as type", function (done) {
            var loadAsset = sinon.stub(Fire.AssetLibrary, "loadAsset", function (uuid, callback) {
            ...
            Editor.setDeepPropertyByPath(target, 'textureAsset', {
                uuid: DummyUuid
            }, Fire.JS.getClassName(Fire.Texture));

            Fire.AssetLibrary.loadAsset.restore();
        });

        //Editor.setDeepPropertyByPath(obj, 'target.textureUrl', '54646542313', Fire.JS.getClassName(Fire.Texture));
    });

    describe("Array", function () {
        var fireObj = new (Fire.Class({
            properties: {
                indices: {
                    default: [],
                    type: Fire.Float
        ...
        describe('Editor.fillDefaultValue', function () {
            ...
            test(Fire.Float, 0);
            test(Fire.Integer, 0);
            test(Fire.Boolean, false);
            test(Fire.String, "");
            test(Fire.defineEnum({
                foo: 1,
                bar: 2
            }), 1);
            test(Fire.Vec2, new Fire.Vec2());
            test(Fire.Asset, null);
        });
    });
    ...
    describe('Testing if property is typed object', function () {
        ...
        it("should set property deeply if path not contains '.'", function() {
            ...
            var value = new Fire.FObject();
            Editor.setDeepPropertyByPath(target, 'value', value, Fire.JS.getClassName(value));
        ...
        it("should set property deeply if path contains one '.'", function () {
            ...
            var value = new Fire.FObject();
            Editor.setDeepPropertyByPath(target, 'foo.bar', value, Fire.JS.getClassName(value));
